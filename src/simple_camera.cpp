...
int main()
{
    ...
    if (!face_cascade.load("./haarcascade_frontalface_default.xml")) // Update with correct path
    {
        // error handling
    ...
    while (true)
    {
        ...
        if (elapsed.count() >= WAIT_TIME) // 5 seconds
        {
            ...
            for (const auto &face : faces)
            {
                ...
                // save the image
                std::string filename = "../Images/image_" + std::to_string(i) + "_" + current_time_formatted() + ".jpg";
