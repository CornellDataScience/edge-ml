...
IS_COLAB = False  # os.name == "posix"
LOAD_DATA = True  # not (os.name == "posix")
IS_EXPERIMENT = False
train_name = "train"
test_name = "test"
WIDTH = HEIGHT = 105
...
def run_combination(l, bs, ep, pat, md, seed, train_path, test_path):
    ...
    parameters_name = (
        f"seed_{seed}_lr_{l}_bs_{bs}_ep_{ep}_val_{validation_size}_"
    ...
    siamese.fit(
        weights_file=load_weights_path,
...
def run():
    ...
    train_path = os.path.join(
        data_path, f"{train_name}.{data_set_save_type}"
    )  # A path for the train file
    test_path = os.path.join(
        data_path, f"{test_name}.{data_set_save_type}"
    )  # A path for the test file
